...
class Points():
    # *vectors 是 tuple
    def __init__(self, *vectors, color=black, show_coordinates=False):
...
def draw(*objects, origin=True, originMark=False, axes=True, grid=(1,1), nice_aspect_ratio=True,
        ...
        if type(object) == Polygon:
            ...
            if object.show_coordinates:
                for vec in object.vectors:
                    plt.annotate('({}, {})'.format(vec[0], vec[1]), (vec[0], vec[1]), xytext=(vec[0]+0.1, vec[1]+0.1))
        elif type(object) == Arrow:
